...
function hoistVariableDeclarator(ex) {
  ...
  const referencedIdentifiers = (0, _findIdentifiers.default)([ex], 'referenced');
  referencedIdentifiers.forEach(identifier => {
    if (identifier.isIdentifier()) {
      hoistIdentifier(identifier);
    }
  });
  const bindingIdentifiers = (0, _findIdentifiers.default)([ex], 'binding');
...
function extractExpression(ex, evaluate = false, imports = []) {
  ...
  if (!evaluated) {
    ...
    (0, _findIdentifiers.default)([ex], 'referenced').forEach(id => {
