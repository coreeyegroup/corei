    ...
    Identifier(identifier) {
      if (identifier.isBindingIdentifier()) {
        ...
        if (parent.isObjectProperty({
          ...
          const chain = getAncestorsWhile(parent, p => p !== objectPattern).filter((0, _isTypedNode.default)('ObjectProperty')).map(p => {
            const key = p.get('key');
            if (!key.isIdentifier()) {
...
function importFromVariableDeclarator(path, isSync) {
  ...
  if (!isSync) {
    ...
    (0, _logger.warn)('evaluator:collectExportsAndImports', '`import` should be awaited');
    return [];
  }
...
function collectFromRequire(path, state) {
  if (!(0, _isRequire.default)(path)) return;
  const {
    parentPath: callExpression
  } = path;
  if (!callExpression.isCallExpression()) {
  ...
  if (container.isCallExpression() && key === 0) {
    ...
    if (!imported) {
      ...
      (0, _logger.warn)('evaluator:collectExportsAndImports', 'Unknown wrapper of require', container.node.callee);
      return;
    }
    ...
    if (!variableDeclarator.isVariableDeclarator()) {
      ...
      (0, _logger.warn)('evaluator:collectExportsAndImports', 'Unexpected require inside', variableDeclarator.node.type);
      return;
    }
    const id = variableDeclarator.get('id');
    if (!id.isIdentifier()) {
      (0, _logger.warn)('evaluator:collectExportsAndImports', 'Id should be Identifier', variableDeclarator.node.type);
      return;
    }
    if (imported === 'import:*') {
  ...
  if (container.isMemberExpression()) {
    ...
    if (!property.isIdentifier() && !property.isStringLiteral()) {
      (0, _logger.warn)('evaluator:collectExportsAndImports', 'Property should be Identifier or StringLiteral', property.node.type);
      return;
    }
    const {
      parentPath: variableDeclarator
...
function getGetterValueFromDescriptor(descriptor) {
  const getter = descriptor.get('properties').filter((0, _isTypedNode.default)('ObjectProperty')).find(p => p.get('key').isIdentifier({
    name: 'get'
  }));
...
function collectFromRequireOrExports(path, state) {
  if ((0, _isRequire.default)(path)) {
    collectFromRequire(path, state);
  } else if ((0, _isExports.default)(path)) {
...
function collectFromAssignmentExpression(path, state) {
  ...
  } else if ((0, _isExports.default)(left)) {
    exported = '*'; // maybe
  }

  if (!exported) return;
  if (!right.isCallExpression() || !(0, _isRequire.default)(right.get('callee'))) return;
...
function collectFromExportStarCall(path, state) {
  ...
  if (!(0, _isRequire.default)(callee) || !sourcePath.isStringLiteral()) return;
  const source = sourcePath.node.value;
  if (!source) return;
  state.reexports.push({
    exported: '*',
...
function collectFromEsbuildReExportCall(path, state) {
  ...
  if (!(0, _isExports.default)(exports)) return;
  const callee = requireCall.get('callee');
  if (!(0, _isRequire.default)(callee)) return;
