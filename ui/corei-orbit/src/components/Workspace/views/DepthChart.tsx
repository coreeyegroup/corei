...
const DepthChart: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Simulate order book data (bids/asks)
    const levels = 30;
    ...
    // Normalize volumes to fit height
    ...
    const bidColor = '#00E5A0';
    const askColor = '#FF4D4D';

    ctx.fillStyle = bidColor;
    bidPrices.forEach((price, i) => {
      const x = padding + (i / (levels - 1)) * chartW * 0.5;
      const y = padding + chartH - (bidVolumes[i] / maxVol) * chartH;
    ...
    askPrices.forEach((price, i) => {
      ...
      const y = padding + chartH - (askVolumes[i] / maxVol) * chartH;
      const barW = chartW * 0.5 / levels * 0.8;
    ...
    ctx.fillText(`Bids (${bidVolumes.reduce((a, b) => a + b, 0).toFixed(2)})`, padding + chartW * 0.25, h - 2);
    ctx.fillText(`Asks (${askVolumes.reduce((a, b) => a + b, 0).toFixed(2)})`, padding + chartW * 0.75, h - 2);
