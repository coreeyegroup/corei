...
interface ProviderFormData {
  ...
  /* Credential schema only.
   * Secret values are NEVER entered here.
   */
...
export const BrokerProviders: React.FC = () => {
  ...
  const [isSubmitting, setIsSubmitting] =
    useState(false);
  /*
   * ADD PROVIDER FLOATING WINDOW
   *
   * Movable through the title bar.
   * Native CSS resize is disabled.
   */
  const [addProviderPosition, setAddProviderPosition] =
    useState({ x: 0, y: 72 });

  const addProviderDragRef = useRef<{
    offsetX: number;
    offsetY: number;
  } | null>(null);
  ...
  const buildInspectorForm = (
    ...
    return {
      ...
      adapterName: displayValue(
        provider?.adapterName ??
          provider?.adapter_name,
        INITIAL_FORM.adapterName,
      ),

      adapterVersion: displayValue(
        provider?.adapterVersion ??
          provider?.adapter_version,
        INITIAL_FORM.adapterVersion,
      ),
  ...
  useEffect(() => {
    ...
    const stop = () => {
      addProviderDragRef.current = null;
  ...
  const saveInspector = async () => {
    ...
    const name =
      inspectorFormData.name.trim();

    const adapterVersion =
      inspectorFormData.adapterVersion.trim();

    const region =
      inspectorFormData.region.trim();
    ...
    if (!adapterVersion) {
      toast.error('Adapter version is required.');
      return;
    }

    if (!region) {
      toast.error('Provider region is required.');
      return;
    }
    ...
    try {
      const existingMetadata =
        (selectedProvider as typeof selectedProvider & { metadata?: Record<string, unknown> })?.metadata &&
        typeof (selectedProvider as typeof selectedProvider & { metadata?: Record<string, unknown> }).metadata ===
          'object'
          ? (selectedProvider as typeof selectedProvider & { metadata?: Record<string, unknown> }).metadata
          : {};
      ...
      await brokerIntelligenceService.addProvider({
        ...
        name,
        adapterName:
          inspectorFormData.adapterName,
        adapterVersion,
        type: inspectorFormData.type,
        region,
        environment:
          inspectorFormData.environment,
        enabled:
          inspectorFormData.enabled,

        metadata: {
          ...existingMetadata,

          apiBaseUrl:
            inspectorFormData.apiBaseUrl.trim(),

          websocketUrl:
            inspectorFormData.websocketUrl.trim(),

          credentialSchema:
            inspectorFormData.credentialFields
              .split('\\n')
              .map(
                (value) =>
                  value.trim(),
      ...
      const refreshedProviders =
        useBrokerIntelligenceStore.getState()
          .providers;

      const refreshedProvider =
        refreshedProviders.find(
          (provider: any) =>
            displayValue(
              provider?.id,
              '',
      ...
      <section className="bi-providers-shell">
        ...
        <div className="bi-providers-content">
          ...
          {selectedProvider && (
            ...
            {selectedProvider && (
              <>
                <header className="bi-inspector-header">
                  <div className="bi-inspector-heading">
                    <span className="bi-inspector-eyebrow">
                      PROVIDER INSPECTOR
                    </span>
                    ...
                    <code className="bi-inspector-id">
                      {displayValue(
                        selectedProvider.id,
                      )}
                    </code>
                  ...
                  {!isInspectorEditing ? (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setInspectorBaseline(
                          nextForm,
                        );
                        setIsInspectorEditing(
                          true,
                        );
                      }}
                    >
                ...
                <div className="bi-inspector-body">
                  <section className="bi-inspector-section">
                    <div className="bi-inspector-section-title">
                      01 — PROVIDER IDENTITY
                    </div>

                    <div className="bi-inspector-grid">
                      <div className="bi-inspector-field">
                        <span>PROVIDER ID</span>
                        <input
                          name="id"
                          value={
                            inspectorFormData.id
                          }
                          readOnly
                          disabled
                          className="is-mono"
                      ...
                      <div className="bi-inspector-field">
                        ...
                        {isInspectorEditing ? (
                          <input
                            name="name"
                            value={
                              inspectorFormData.name
                            }
                            onChange={
                              handleInspectorInputChange
                            }
                            autoComplete="off"
                          />
                        ) : (
                      ...
                      <div className="bi-inspector-field">
                        ...
                        {isInspectorEditing ? (
                          <input
                            name="adapterVersion"
                            value={
                              inspectorFormData.adapterVersion
                            }
                            onChange={
                              handleInspectorInputChange
                            }
                  ...
                  <section className="bi-inspector-section">
                    ...
                    <div className="bi-inspector-state-line">
                      <span>STATE</span>
                      <strong>
                        {displayValue(
                          selectedState,
                      ...
                      <div className="bi-inspector-field">
                        ...
                        {isInspectorEditing ? (
                          <input
                            name="websocketUrl"
                            value={
                              inspectorFormData.websocketUrl
                            }
                            onChange={
                              handleInspectorInputChange
                            }
                  ...
                  <section className="bi-inspector-section">
                    ...
                    <div className="bi-inspector-state-line">
                      ...
                      <strong className="is-mono">
                        {displayValue(
                          (selectedProvider as typeof selectedProvider & { credentialRef?: string })?.credentialRef ??
                            (selectedProvider as typeof selectedProvider & { credential_id?: string })?.credential_id,
                          'VAULT-MANAGED',
                        )}
                      </strong>
                    </div>
                    ...
                    <div className="bi-inspector-vault-actions">
                      <a
                        href={
                          import.meta.env
                            .VITE_VAULT_UI_URL ||
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="bi-inspector-vault-link"
                        onClick={(event) => {
                          if (
                            !import.meta.env
                              .VITE_VAULT_UI_URL
                            ...
                            toast.error(
                              'VITE_VAULT_UI_URL is not configured.',
                            );
                          }
                        }}
                      >
                    ...
                    <div className="bi-inspector-note">
                      ...
                      or edited here. Vault remains the
                      authoritative credential boundary.
                    </div>
                  </section>

                  <div className="bi-inspector-note">
                    Configuration fields and options
                    mirror ADD PROVIDER. Runtime state
                    is observed here and remains
                    read-only.
                  </div>
                </div>
              </>
      ...
      {isAddModalOpen && (
        ...
        >
          <section
            ...
            style={{
              left: `${addProviderPosition.x}px`,
              top: `${addProviderPosition.y}px`,
            }}
            onMouseDown={(event) =>
          ...
          >
            <header
              className="bi-provider-modal-header"
              onMouseDown={
                beginAddProviderDrag
              }
            >
              <div>
                <span className="bi-providers-eyebrow">
                ...
                <section className="bi-provider-form-section">
                  ...
                  <div className="bi-provider-form-grid">
                    <FormField label="VAULT PATH">
                      <input
                        value={
                          `secret/trading/broker/${formData.id.trim() || '<provider-id>'}`
                        }
                        readOnly
                      />
                    </FormField>

                    <FormField label="REQUIRED FIELDS">
                      <textarea
                  ...
                  <div className="bi-provider-vault-row">
                    <span>
                      VAULT UI
                    </span>
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="bi-provider-vault-link"
                      onClick={(event) => {
                        if (
                          !import.meta.env
                            .VITE_VAULT_UI_URL
